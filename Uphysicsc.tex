...
In the last section it has been shown that it is quite hard to arrive at Mars orbit at a close position to the planet. This raise some issues regarding not only the possibility of encounter with Mars but also whether craft speed is safe enough or not. The idea is further on given consists in trying to stabilize the solar sail at Mars orbit. Being succesful with this would implie a similar velocity to Mars orbital one, and moreover, the guarantee of finding the planet since phase difference can be corrected as explained in Section 3.1.2.
\clearpage
