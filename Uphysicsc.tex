...
\subsection{Restatement of the problem}
 A more detailed interpretation of the problem has been made to achieve an optimized flight plan. Understanding firstly the physical foundations around solar sailing was essential in order to perform an accurate analysis.  reasoning the total force exerted on the sail and being able to compute results regarding trajectories. Since some initial conditions are already given it has been necessary to verify whether these were the best options to accomplish the overall goal of the study or not.
 
 Moreover the aim of the survey was not only to calculate the path followed by the craft but also to determine its velocity so that a safe landing can be guaranteed. However, as trying to consider the attraction of Mars does not always give accurate results, this is not an easy task, though an estimate of craft arrival to Mars is presented at the end.
...
\subsection{The logarithmic spiral trajectory}
Literature suggests \cite{tsu} one can neglect the  $\dot{v}_r$ term in equation !!! as a first approach. The simplified equation reads
\begin{equation}
